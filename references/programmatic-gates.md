...
- 默认章节数量为 3-6；每张章节过渡页后至少有一张内容页，内容页不得出现在第一张章节过渡页之前，目录与结尾之间不得混入其他页面类型。
- 目录页至少包含 1 个目录项。
- 目录列表项总数与章节过渡页数量完全一致，章节序号连续。
- LLM 草稿只包含文本和列表结构；内容校验通过后，列表序号、章节序号、页面级图片和语义图标按确定性规则绑定，再执行完整输入校验。
...
- 所有必填文本、图片和图标节点均有值；序号字段可省略，由确定性流程绑定。
